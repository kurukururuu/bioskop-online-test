      ...
      <div v-touch:tap="e => clickActiveOnly(e, item)" class="relative">
        ...
        <div class="absolute top-0 left-0 w-full h-full flex flex-col justify-center p-8 mobile:hidden">
          ...
          <div class="details text-white z-10">
            <img :src="item.cover.logo" alt="title" class="mb-4 w-36">
            <div class="text-sm mb-7"><span class="text-yellow-primary font-bold">{{ formatter.format(item.price.normal) }}</span> ( Belum termasuk pajak )</div>
        ...
        <div class="absolute bottom-8 left-0 w-full desktop:hidden">
          <img :src="item.cover.logo" alt="title" class="mx-auto">
        </div>
      </div>
    </div>
  </VueSlickCarousel>
